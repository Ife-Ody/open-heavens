...
import { Dispatch, SetStateAction, createContext, useCallback, useContext, useEffect, useId, useState } from 'react'
...
export function KeyboardShortcutProvider({ children }: { children: React.ReactNode }) {
    ...
    return <KeyboardShortcutContext.Provider value={{ listeners, setListeners }}>{children}</KeyboardShortcutContext.Provider>
}

export function useKeyboardShortcut(key: KeyboardShortcutListener['key'], callback: (e: KeyboardEvent) => void, options: Pick<KeyboardShortcutListener, 'enabled' | 'priority' | 'modal' | 'sheet'> = {}) {
    const id = useId()
    ...
    const onKeyDown = useCallback(
        (e: KeyboardEvent) => {
            ...
            if (target.tagName === 'INPUT' || target.tagName === 'TEXTAREA' || !!existingModalBackdrop !== !!options.modal || !!existingSheetBackdrop !== !!options.sheet) return

            const pressedKey = [...(e.metaKey ? ['meta'] : []), ...(e.ctrlKey ? ['ctrl'] : []), ...(e.altKey ? ['alt'] : []), e.key].join('+')
            ...
            const matchingListeners = listeners.filter((l) => l.enabled !== false && !!existingModalBackdrop === !!l.modal && !!existingSheetBackdrop === !!l.sheet && (Array.isArray(l.key) ? l.key.includes(pressedKey) : l.key === pressedKey))
