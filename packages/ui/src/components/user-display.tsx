...
const UserDisplay = ({ user, className, size = 'md', avatar = true, username = true, name = true, avatarClassName = '', nameClassName = '', usernameClassName = '' }: UserDisplayProps) => {
    if (!user?.username) {
        return <></>
    }
        ...
        <div className={cn('flex items-center gap-3 relative', className)}>
            <div className='relative'>{avatar && (
                <Avatar className={avatarClassName}>
                    <AvatarImage src={user?.profile?.avatar || `https://api.dicebear.com/8.x/thumbs/svg?seed=${user?.username}`} alt={user?.username} />
                    <AvatarFallback>
                        {user?.profile?.firstname?.[0] && user?.profile?.lastname?.[0] ? user?.profile?.firstname?.[0] + user?.profile?.lastname?.[0] : user?.username?.charAt(0)}
                    </AvatarFallback>
                </Avatar>
            )}
            ...
            <div className="my-auto text-sm truncate flex-0">
                {name && (
                    <div className={cn('font-medium', nameClassName)}>
                        {user?.profile?.firstname?.[0] && user?.profile?.lastname?.[0] ? `${user?.profile?.firstname} ${user?.profile?.lastname}` : user?.username}
                    </div>
                )}
                {username && <div className={cn('text-xs text-muted-foreground', usernameClassName)}>{user?.username}</div>}
            </div>
        </div>
    )
}
